#pragma once
#include "Logger.hpp"
#include "helpers/TaskComparators.hpp"
...
private:
  ...
  std::priority_queue<Task *, std::vector<Task*>, TaskPriorityComparator> readyTasks;
  std::priority_queue<Task *, std::vector<Task*>, TaskTimeComparator> scheduledTasks;
...
public:
  ...
  // std::vector<std::pair<std::string, TaskStatus>> getAllTasksStatus();
  // std::vector<std::pair<std::string, TaskStatus>> getTasksByStatus(TaskStatus status);
