...
TaskFactory::TaskFactory(int defaultMaxRetries) : defaultMaxRetries(defaultMaxRetries) {}
      ...
      if (ss >> mode) {
        if (mode == "H") {
          ...
          if (startTime) {
            ...
            // auto diff = std::chrono::duration_cast<std::chrono::minutes>(*startTime - now).count();
            // std::cout << "Parsed time is " << diff << " minutes from now\n";
          }
        } else if (mode == "C") {
          ...
          cycleInterval = std::chrono::seconds(std::stoi(cycleIntervalStr));
        }
      }
    } else {
      mode = nextToken;
